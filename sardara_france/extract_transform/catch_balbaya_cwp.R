...
####################### Catch according to spatiotemporal resolution from Balbaya database
# Author : Chloé Dalleau, M2 intern (IRD)
# Training supervisor : Paul Taconet (IRD)
# Date : 19/09/2017 
# 
# ## Purpose
# To determine the quantity of catches according to spatiotemporal resolution from Balbaya database.
#
# ## Note
...
# - area : geometric coordinates in WKT of the spatial grid extent
# - cent_area : centroid of each areas
# - species : ID of species type (FAO code, three letters). example : "SKJ"
...
# 
# ## WPS code
# - wps.des: id = catchaggregatebalbaya, title = Tuna catches according to spatiotempral resolution from Balbaya database, abstract = To determine catches by spatiotemporal resolution from Balbaya database.;
...
# - wps.out: id = catchaggregatebalbayacsv, type = text/zip, title = Catches from Balbaya database aggregating according to spatiotemporal resolution ; 
#########################
...
### Processing of data (No choice in this script version, see: catch_balbaya_rand_eqd.R )
...
query<- paste("
              ...
              ROW_NUMBER() OVER (ORDER BY activite.c_ocea, activite.c_bat,activite.d_act, activite.n_act) AS id_set,
              CASE
              ...
              WHEN type_banc.l4c_tban = 'IND' THEN 'UNK'::text -- indéterminé, unknown
              END AS schooltype, 
              capture.c_cat_p, -- catégorie de poids
              CASE
              -- ATTENTION les espèces considérées en tant que rejet on un code espèce compris dans l'interval [8, 800:899]
              WHEN espece.c_esp = 8::numeric THEN 'D'::text --Discard, rejeté
              WHEN (espece.c_esp >= 800 AND espece.c_esp <= 899) THEN 'D'::text --Discard, rejeté
              ELSE 'L'::text -- Landing
              END AS catchtype, 
              ROUND(capture.v_poids_capt * activite.rf3,3) AS catch,-- rf3 : ratio de capture
              'MT'::text AS catchunit, -- unité de capture fixe :tonne
              activite.v_la_act, 
              activite.v_lo_act, 
              activite.the_geom
              FROM 
              ...
              (activite.d_act BETWEEN '",firstDate,"'::date AND '",finalDate,"'::date)
...
# - catch : quantity of catches. example : "1.02"
names(dataset) <- c("id_set", "ocean", "flag", "gear", "c_bat",	"d_act", "n_act", "the_geom",
                    "lat_data", "lon_data", "time_start",	"time_end",	"area", "cent_area",
                    "schooltype", "species", "catchtype","c_cat_p", "catchunit", "v_catch")
cat("Extract data ... ok \n")
...
### Aggregation of data
# list of dimensions
dimensions <- c("ocean", "flag", "gear", if(agg_with_boat ==TRUE){"c_bat"} , "time_start",
"time_end", "area", "cent_area", "schooltype", "species", "catchtype", "catchunit")
...
dataset_name <- as.character(paste("indian_atlantic_oceans_catch_",spatialStep_filename,"deg_",timeStep,timeUnit_metadata,"_",first_date_dataset,"_",last_date_dataset,"_",method_asso_metadata,"_balbaya", sep=""))
dataset_release_date <- as.character(Sys.Date())
dataset_title <- as.character(paste("Catch of tuna and tuna-like species in the Atlantic and Indian oceans (",first_year_dataset, "-", last_year_dataset, ") for french surface fisheries by ",spatialStep_filename, "° / ",timeStep,timeUnit_metadata,"square (logbooks)", sep=""))
...
# steps
step_metadata <- paste(
  "step1: Catch data from Balbaya database were collated and harmonized.
