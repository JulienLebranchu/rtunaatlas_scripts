...
####################### Workflow to setup and update the French Tropical tuna atlas database
# Author : Paul Taconet (IRD), inpired from Chloe Dalleau (IRD)
# Date : 15/02/2018 
# 
...
# wps.in: id = db_ob7_user_name, type = character, title =  user name to connect to the database with read credentials , value = "NULL"
# wps.in: id = db_ob7_user_password, type = character, title =  passowrd for db_ob7_user_name , value = "NULL"
# wps.in: id = sql_limit, type = integer, title = SQL limit for the query (useful for tests). Put NULL for no limit, value = "1000";
...
#### TO REMOVE wps.in: id = file_path_metadata_model, type = character. title = File path of the metadata model;
...
parameter_bdd$user <- db_ob7_admin_name
parameter_bdd$password <- db_ob7_admin_password
