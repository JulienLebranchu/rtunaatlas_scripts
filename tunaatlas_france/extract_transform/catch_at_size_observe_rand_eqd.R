...
####################### Catch at size according to spatiotemporal resolution from Observe database
# Author : Chloé Dalleau, M2 intern (IRD)
# Training supervisor : Paul Taconet (IRD)
# Date : 21/09/2017 
# 
# ## Purpose
# To determine the catch at size according to spatiotemporal resolution from Observe database.
# 
# ## Note
...
# - finalDate : final date of calendar, type date. Advice value : "2100-01-01" or Sys.Date() (current date).
...
# - size_measure_type : list of size meseasures types wanted, type character.
...
# - agg_with_size_type : optional dimension: size measure type, type = boolean. Value : "TRUE|FALSE"
# 
# ## Output data
# catch at size by :
...
# - size_type : optional. size measure type. example : "total length" "eye-fork length"
...
# 
# ## WPS code
# - wps.des: id = catchatsiezaggregateobserve, title = Tuna catch at size according to spatiotempral resolution from Observe database, abstract = To determine the catch at size by spatiotemporal resolution from Observe database;
...
# - wps.in: id = size_measure_type, type = character, title = list of size meseasures types wanted, type character., value = "TL";
...
# - wps.in: id = agg_with_size_type, type = boolean. title = optional dimension: size measure type. Value : "TRUE|FALSE"
# - wps.out: id = catchatsizeaggregateobservecsv, type = text/zip, title = Catch at size from Observe database according to spatiotemporal resolution; 
#########################
...
### Processing of data (to use CWP method, see: catch_at_size_observe_cwp.R )
...
## WARNING : if 'agg_with_size_type' is FALSE, all measures types will be merged
...
## Note : all size measures types are extract to database (SQL language), the selection of them are done in 'treatments' part (R language)
...
agg_with_program = TRUE
### boolean : if you want an aggregation with size measure type put TRUE
agg_with_size_type = FALSE
...
query<- paste("
              ...
              SELECT
              union_catch_PS.id_set AS n_set,
              CASE 
              WHEN observe_common.ocean.code='1'::text THEN 'ATL'
              WHEN observe_common.ocean.code='2'::text THEN 'IND'
              WHEN observe_common.ocean.code='3'::text THEN 'PAC'
              ...
              -------------------------------------------------------------------------
              data_by_area AS (
              -- détail des calées par polygone
              -- répétition de la calée si associé à plusieurs polygones 
              SELECT 
              ...
              UNION_LL_PS.species, 
              UNION_LL_PS.sex,
              UNION_LL_PS.sizeinterval,
              UNION_LL_PS.sizemin,
              UNION_LL_PS.sizetype,
              ...
              -- détail des calées associées à un seul et unique polygone
              SELECT 
              area_process.ID,
              data_by_area.ocean,
              data_by_area.flag, 
              ...
              -------------------------------------------------------------------------
              -- Detail des calées par polygones et par période
              -------------------------------------------------------------------------
              
              -- détail des calées selon la grille spatio-temporelle
              SELECT 
              ...
              set_by_select_area.sex,
              set_by_select_area.sizeinterval,
              set_by_select_area.sizemin,
              set_by_select_area.sizetype,
...
# - catchunit : catch unit. example : "NO" for number of fish
# - catch : quantity of catches. example : "1.02"
names(dataset) <- c("ID", "ocean", "flag", "program", "gear", "c_bat", "d_set", "the_geom", "lat_data", "lon_data",	"time_start",	"time_end",	"area", "cent_area", "schooltype", "species", "sex", "size_step", "size_min", "size_type", "catchtype", "catchunit", "v_catch")
cat("Extract data ... ok \n")
...
### Size measure type extraction
dataset_table_size <- NULL
for (i in 1:length(size_measure_type)){
  dataset_table_size <- rbind(dataset_table_size,subset(dataset_table, dataset_table$size_type==size_measure_type[i]))
}
dataset_table <- dataset_table_size

### Aggregation of data
# list of dimensions
dimensions <- c("ocean", "flag", if(agg_with_program ==TRUE){"program"}, "gear",
                if(agg_with_boat ==TRUE){"c_bat"} , "time_start", "time_end",
                "area", "cent_area", "schooltype", "species", "sex", "size_step", "size_min",
                if(agg_with_size_type ==TRUE){"size_type"}, "catchtype", "catchunit")
...
if (agg_with_boat == TRUE) {
  extra_info = "The boat code fit vessel keel code. "
} else {
  extra_info = ""
} 
## add in metadate information about size measures type
# data.frame with all dimensions : code and label
all_label <- data.frame(c("L1" , "FL" , "CFL" ,"PAL" , "TL" ,"SL" , "LJFL" ,"CLJFL", "IDL" ,
                          "EFL" ,  "DW" ,  "DL" , "TW" , "CTL" ,"DML" ,"CDML" ,"PFL" , "PD1" ,
                          "SCL" ,  "CCL" ,  "SCW" ,  "CCW" ) ,
                        c("L1 (depend of species, see observator manual)","Fork Length","Curved Fork Length",
                          "Pectoral-Anal Length","Total Length",  "Standard Length", "Lower Jaw-Fork Length",
                          "Curved Lower Jaw-Fork Length", "Interdorsal Length", "Eye-Fork Length", "Disk Width",
                          "Disk Length", "Total Weight",  "Curved Total Length", "Dorsal Mantle Length",
                          "Curved Dorsal Mantle Length",  "Pectoral-Fork Length","Predorsal Length", "Straight Carapace Length",
                          "Curved Carapace Length",  "Straight Carapace Width", "Curved Carapace Width"),
                        stringsAsFactors = F)
colnames(all_label)<-c("code", "label")
# list of size type in the dataset
list_size_type <- NULL
data_size_measure_type <- levels(as.factor(dataset_table_size$size_type))
# character string with list of size type in the dataset
list_size_type <- paste( all_label[which(all_label$code==data_size_measure_type[1]),2], sep = "")
for (i in 2:length(data_size_measure_type)){
  list_size_type <- paste(list_size_type," , ", all_label[which(all_label$code==data_size_measure_type[i]),2], sep = "")
}
extra_info = paste(extra_info,"Size measures types are: ",list_size_type, ".", sep = "" )
## add warning if agg_with_size_type == False
if (agg_with_size_type == FALSE) {
  extra_info = paste(extra_info,". Warning : All size measures types are merged and a fish can be measured several times with different measures types.")
} 
...
dataset_name <- as.character(paste("indian_atlantic_oceans_catch_at_size_",spatialStep_filename,"deg_",timeStep,timeUnit_metadata,"_",first_date_dataset,"_",last_date_dataset,"_",method_asso_metadata,"_observe", sep=""))
dataset_release_date <- as.character(Sys.Date())
dataset_title <- as.character(paste("Catch at size of tuna and tuna-like species in the Atlantic and Indian oceans (",first_year_dataset, "-", last_year_dataset, ") for french surface fisheries by ",spatialStep_filename, "° / ",timeStep,timeUnit_metadata,"square (observers and logbooks)", sep=""))
...
step_metadata <- paste(
  ...
  step5: Each fishing data was associated to ", if (method_asso=="equaldistribution"){"one or several polygons"} else {"one polygon"} ," using the data geolocalisation.", description_method_asso," Note:  for longline sets, the centroid is used to combine to polygon.
...
cat("- optional dimension: aggregation with size measure type ", agg_with_size_type, "\n")
