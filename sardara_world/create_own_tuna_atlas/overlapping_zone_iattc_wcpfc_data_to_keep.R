...
if (overlapping_zone_iattc_wcpfc_data_to_keep=="IATTC"){
  ...
  df<-df[ which(!(df$geographic_identifier %in% overlapping_zone_iattc_wcpfc$codesource_area & georef_dataset$source_authority == "WCPFC")), ]
  ...
  df<-df[ which(!(df$geographic_identifier %in% overlapping_zone_iattc_wcpfc$codesource_area & georef_dataset$source_authority == "IATTC")), ]
}
