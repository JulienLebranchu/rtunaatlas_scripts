...
######################################################################
# wps.des: id = create_own_tuna_atlas, title = Create your own version of the Tuna Atlas, abstract = This algorithm allows to create own regional or global tuna altas. It takes as input the public domain datasets of the five Tuna Regional Fisheries Management Organizations (IOTC|ICCAT|WCPFC|IATTC|CCSBT) stored within the Sardara database. It proposes a set of parameters to customize the computation of the tuna atlas. In addition to the output dataset it returns a set of information regarding the transformations that are made during the processing. ;
# wps.in: id = include_IOTC, type = boolean, title = Include data from IOTC in the atlas (TRUE or FALSE), value = TRUE;
# wps.in: id = include_ICCAT, type = boolean, title = Include data from ICCAT in the atlas?, value = TRUE;
# wps.in: id = include_IATTC, type = boolean, title = Include data from IATTC in the atlas?, value = TRUE;
# wps.in: id = include_WCPFC, type = boolean, title = Include data from WCPFC in the atlas?, value = TRUE;
# wps.in: id = include_CCSBT, type = boolean, title = Include data from CCSBT in the atlas?, value = TRUE;
...
# wps.in: id = overlapping_zone_iattc_wcpfc_data_to_keep, type = string, title = Concerns IATTC and WCPFC data. IATTC and WCPFC have an overlapping area in their respective area of competence. Which data should be kept for this zone? IATTC : keep data from IATTC. WCPFC : keep data from WCPFC., value = "IATTC|WCPFC";
# wps.in: id = SBF_data_rfmo_to_keep, type = string, title = Concerns Southern Bluefin Tuna (SBF) data. Use only if parameter include_CCSBT is set to TRUE. SBF tuna data do exist in both CCSBT data and the other tuna RFMOs data. Wich data should be kept? CCSBT : CCSBT data are kept for SBF. other_trfmos : data from the other TRFMOs are kept for SBF., value = "CCSBT|other_trfmos";
...
gear_filter="09.1,09.2,01.1,01.2"
...
cat("Retrieving primary datasets from Sardara DB... \n")
...
if (mapping_map_code_lists==TRUE){
  ...
  function_map_dataset_codelists<-function(dataset_to_map,mapping_dataset){
    ...
    for (i in 1:length(dimensions_to_map)){ # i takes the values of the dimensions to map
      if (dimension_to_map[i] %in% colnames(dataset_to_map)){
        ...
        dataset_to_map<-rtunaatlas::map_codelist(dataset_to_map,df_mapping_final_this_dimension,dimension_to_map[i])$df  # Codes are mapped by tRFMOs (source_authority) 
      }
    }
  ...
  catch<-function_map_dataset_codelists(catch,mapping_dataset)
  cat("Mapping code lists of georeferenced catch datasets OK\n")
  
  
  if(raising_georef_to_nominal==TRUE){
    cat("Mapping code lists of nominal catch datasets...\n")
    nominal_catch<-function_map_dataset_codelists(nominal_catch,mapping_dataset)
...
if (unit_conversion_convert==FALSE){      # If the user decides not to convert catchunits, nothing is done.
  ...
  catch<-convert_units(df_input = catch,
...
if (raising_georef_to_nominal==FALSE) {   # We do not raise the data 
  ...
  if ( include_CCSBT==TRUE | include_WCPFC==TRUE ) {
    ...
    stats_WCPFC_CCSBT_raised<-catch_WCPFC_CCSBT_raised$stats
  } else {
    catch_WCPFC_CCSBT_raised<-NULL
    stats_WCPFC_CCSBT_raised<-NULL
  }
  ...
  if ( include_IOTC==TRUE | include_ICCAT==TRUE | include_IATTC==TRUE ) {
    ...
    catch_IOTC_ICCAT_IATTC_raised<-catch_WCPFC_CCSBT_raised$df
    stats_raising_IOTC_ICCAT_IATTC<-catch_IOTC_ICCAT_IATTC_raised$stats
  } else {
    catch_IOTC_ICCAT_IATTC_raised<-NULL
    stats_raising_IOTC_ICCAT_IATTC<-NULL
  }
...
#### 6) Spatial Aggregation / Disaggregation of dataf
...
if (disaggregate_on_5deg_data_with_resolution_superior_to_1deg==TRUE) { 
...
#### 7) Curation of data mislocated (i.e. on land areas or without any spatial information) (data with no spatial information have the dimension "geographic_identifier" set to "UNK/IND" or NA)
